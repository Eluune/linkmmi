...
else
{
  ...
  if(isset($_GET["tag"]))
  {
    // récupération des informations posts tag utilisateur
    $requete='SELECT DISTINCT topic.idTopic AS "id", topic.contenuTopic AS "contenu", topic.imgTopic AS "imgTopic", topic.dateTopic AS "date", utilisateur.prenomUser AS "prenom", utilisateur.nomUser AS "nom", utilisateur.atnameUser AS "atname", utilisateur.photoUser AS "imgUser", tag.idTag, tag.nomTag
              FROM topic, utilisateur, tag, reference
              WHERE topic.idUser = utilisateur.idUser AND tag.idTag = reference.idTag AND reference.idTopic = topic.idTopic AND tag.idTag = '.$_GET["tag"].'
  ...
  else if(isset($_GET["tagname"]))
  {
    ...
    $requete='SELECT DISTINCT topic.idTopic AS "id", topic.contenuTopic AS "contenu", topic.imgTopic AS "imgTopic", topic.dateTopic AS "date", utilisateur.prenomUser AS "prenom", utilisateur.nomUser AS "nom", utilisateur.atnameUser AS "atname", utilisateur.photoUser AS "imgUser", tag.idTag, tag.nomTag
              FROM topic, utilisateur, tag, reference
              WHERE topic.idUser = utilisateur.idUser AND tag.idTag = reference.idTag AND reference.idTopic = topic.idTopic AND tag.nomTag LIKE "%'.$_GET["tagname"].'%"
              ORDER BY topic.dateTopic DESC';
    $resultats=$bdd->query($requete);
    $posts=$resultats->fetchAll(PDO::FETCH_OBJ);
    $resultats->closeCursor();
  ...
  {
    // récupération des informations utilisateur comprenant le atname...
    $requete='SELECT DISTINCT utilisateur.prenomUser AS "prenom", utilisateur.nomUser AS "nom", utilisateur.atnameUser AS "atname", utilisateur.photoUser AS "imgUser",
    utilisateur.idUser AS "id", utilisateur.photoUser AS "photo"
              FROM utilisateur, tag, reference
              WHERE utilisateur.atnameUser LIKE "%'.$_GET["at"].'%"';
    $resultats=$bdd->query($requete);
    $posts=$resultats->fetchAll(PDO::FETCH_OBJ);
    $resultats->closeCursor();
  }
  else
  {
    ...
    $requete='SELECT DISTINCT topic.idTopic AS "id", topic.contenuTopic AS "contenu", topic.imgTopic AS "imgTopic", topic.dateTopic AS "date", utilisateur.prenomUser AS "prenom", utilisateur.nomUser AS "nom", utilisateur.atnameUser AS "atname", utilisateur.photoUser AS "imgUser"
    ...
    <div class="section-center">
      <?php foreach ($posts as $post): ?>
        <?php if(isset($_GET["at"])): ?>
          <div class="profils" >
            <div class="utilisateur" data-number="<?php echo $post->id; ?>">

              <div class="photo-utilisateur">
                <?php if(!empty($post->photo) && $post->photo != "NULL"): ?>
                  <img src="<?php echo $url_users.$post->photo ?>" alt="">
                <?php else: ?>
                  <img src="img-placeholder/bestLoutre.jpg">
                <?php endif; ?>
              </div>

              <div class="info-utilisateur">
                <h2><?php echo $post->prenom." ".$post->nom ?></h2>
                <a href="profil.php?user=<?php echo explode('#',$post->atname)[1]; ?>"><?php echo "@"; echo explode('#',$post->atname)[0]; ?></a>
              </div>

              <button type="button" name="button">Suivre</button>
            </div>
          </div>
        <?php else: ?>
          <?php
